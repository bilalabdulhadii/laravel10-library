...
<div class="lg:p-8 bg-white border-b border-gray-200">
...
<div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 gap-6 lg:gap-8 p-6 lg:p-8">
    <table style="margin: 0" class="table table-bordered table-striped">
        ...
        <tbody>
        @if(Auth::user()->overdueOrActiveLoans()->count() > 0)
            @foreach(Auth::user()->overdueOrActiveLoans() as $loan)
                    <tr>
                        <td>{{$loan->id}}</td>
                        <td class="{{ $loan->status == "Active" ? 'active-loan' : 'overdue-loan'}}">{{$loan->status}}</td>
                        <td>{{$loan->userId->name}}</td>
                        <td>{{$loan->bookId->title}}</td>
                        <td>{{$loan->due_date}}</td>
                        <td><a href="{{route('dashboard.loan', ['id' => $loan->id])}}" class="btn btn-danger btn-sm"
                               onclick="return confirm('Are you sure, you want to return the book: {{$loan->id}} .!?')">
                                Return The Book</a>
                    </tr>
            @endforeach
        @else
            <tr>
                <td style="text-align: center" colspan="6">No borrowed books</td>
            </tr>
        @endif
        </tbody>
    </table>
...
<div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 gap-6 lg:gap-8 p-6 lg:p-8">
    <table style="margin: 0" class="table table-bordered table-striped">
        ...
        <tbody>
        @if(Auth::user()->returnedLoans()->count() > 0)
        ...
        @else
            <tr>
                <td style="text-align: center" colspan="5">No History</td>
            </tr>
        @endif
        </tbody>
    </table>
...
<div class="p-6 lg:p-8 bg-white border-b border-gray-200">
    ...
    <h1 class="mt-8 text-2xl font-medium text-gray-900">
        Dashboard to your Jetstream application!
    </h1>
